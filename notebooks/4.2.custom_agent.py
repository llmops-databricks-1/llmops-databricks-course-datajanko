...
request1 = ResponsesAgentRequest(
    input=[{"role": "user", "content": "Calculate 100 plus 50"}], custom_inputs={"session_id": conversation_session, "request_id": f"req-1-{uuid4().hex[:8]}"}
)
...
session_traces_df = mlflow.search_traces(filter_string=f"request_metadata.`mlflow.trace.session` = '{conversation_session}'", order_by=["timestamp_ms ASC"])
