...
client = OpenAI(api_key=w.tokens.create(lifetime_seconds=1200).token_value, base_url=f"{w.config.host}/serving-endpoints")
...
def retrieve_documents(query: str, num_results: int = 5) -> list[dict]:
    ...
    results = index.similarity_search(query_text=query, columns=["text", "title", "arxiv_id", "authors", "year"], num_results=num_results, query_type="hybrid")
...
def rag_query(question: str, num_docs: int = 5) -> dict:
    ...
    return {"question": question, "answer": answer, "sources": [{"title": doc["title"], "arxiv_id": doc["arxiv_id"]} for doc in documents]}
...
class SimpleRAG:
    ...
    def __init__(self, llm_endpoint: str, index_name: str):
        ...
        self.client = OpenAI(api_key=self.w.tokens.create(lifetime_seconds=1200).token_value, base_url=f"{self.w.config.host}/serving-endpoints")
    ...
    def retrieve(self, query: str, num_results: int = 5) -> list[dict]:
        ...
        results = index.similarity_search(query_text=query, columns=["text", "title", "arxiv_id"], num_results=num_results, query_type="hybrid")
