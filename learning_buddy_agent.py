...
"""Self-contained Learning Buddy agent model file for MLflow model serving.

All dependencies are inlined — no imports from the learning_buddy package.
"""
...
# COMMAND ----------
# LearningBuddyVectorSearchManager
...
class LearningBuddyAgent(ResponsesAgent):
    ...
    def _get_problems_by_week(self, course: str, week: int) -> str:
        ...
        raw = self._vs_manager.search("exercise problem", num_results=20, filters={"material_id": material_id})
        chunks = self._parse_vector_search_results(raw)

        if not chunks:
            return json.dumps(
                {
                    "error": f"""No content found for course='{course}', week={week}. Expected material_id='{material_id}'.
        ...
        return json.dumps({"course": course, "week": week, "material_id": material_id, "title": title, "text": full_text}, ensure_ascii=False, indent=2)
    ...
    def _build_tools(self) -> list[ToolInfo]:
        return [
            ToolInfo(name="get_problems_by_week", spec=GET_PROBLEMS_BY_WEEK_SPEC, exec_fn=self._get_problems_by_week),
            ToolInfo(name="search_lectures", spec=SEARCH_LECTURES_SPEC, exec_fn=self._search_lectures),
            ToolInfo(name="search_homework", spec=SEARCH_HOMEWORK_SPEC, exec_fn=self._search_homework),
        ]
    ...
    def call_llm(self, messages: list[dict[str, Any]]) -> Generator[dict[str, Any], None, None]:
        with warnings.catch_warnings():
            ...
            with mlflow.start_span(name="call_llm", span_type=SpanType.LLM) as span:
                ...
                outputs: dict[str, Any] = {"model": last_chunk.get("model"), "usage": last_chunk.get("usage")}
                if llm_request_id:
    ...
    def _extract_output_items(self, events: list[ResponsesAgentStreamEvent]) -> list[dict[str, Any]]:
        items = []
        for e in events:
    ...
    def _run_tool_loop(self, messages: list[dict[str, Any]], max_iter: int = 10) -> list[ResponsesAgentStreamEvent]:
        ...
        for _ in range(max_iter):
            ...
            if last_msg.get("role") == "assistant":
                ...
                events.extend(output_to_responses_items_stream(chunks=self.call_llm(messages), aggregator=messages))
        else:
            events.append(
                ResponsesAgentStreamEvent(
                    type="response.output_item.done", item=self.create_text_output_item("Max iterations reached. Stopping.", str(uuid4()))
                )
            )
        return events
    ...
    def call_and_run_tools(
        ...
        if session_id and self.memory:
            self.save_memory(session_id, request_input + self._extract_output_items(events))
        return events

    def predict(self, request: ResponsesAgentRequest) -> ResponsesAgentResponse:
        events = list(self.predict_stream(request))
        return ResponsesAgentResponse(output=self._extract_output_items(events), custom_outputs=request.custom_inputs)
